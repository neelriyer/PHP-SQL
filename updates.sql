...
--trim every cell TODO
UPDATE movie SET title = TRIM (title);

--convert actor name to upper case
--update actor set name = upper(name);

--convert movie title to upper case (undo this)
--update movie set title = upper(title);

--merge genres for the same movie into one cell (not working) TODO
/*
update genre 
set genre = (select movie_id, string_agg(genre,  ', ') as genre
from genre
group by movie_id)
*/
--
